...
define( 'TW_ABS_URL',  WP_CONTENT_URL . '/plugins/' . plugin_basename( dirname(__FILE__) ) . '/' );
define( 'TW_REL_URL',  dirname( plugin_basename(__FILE__) ) );
define( 'TW_ABS_PATH', WP_PLUGIN_DIR . '/' . plugin_basename( dirname(__FILE__) ) . '/' );

class TwitterWidget extends WP_Widget {

  function TwitterWidget() {
    $widget_opts = array( 'classname' => 'widget_twitter', 'description' => 'Displays last tweets' );
    $this->WP_Widget( 'TwitterWidget', 'Twitter', $widget_opts );
  }
...
function TwitterWidgetInit() {
  wp_register_script('twitter', (TW_ABS_URL . 'js/jquery.tweet.js'), false, '', true);
